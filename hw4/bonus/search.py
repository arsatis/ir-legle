...
def run_search(dict_file, postings_file, query_file, results_file):
    ...
    with open(query_file, 'r') as f, open(results_file, 'w') as r_file:
        ...
        score_id_pairs = list(free_text_model.cosine_score(refined_query, k=3500))

        # Start Pseudo-Relevance Feedback
        prf_k = 5
        prf_terms = []
        for i in range(prf_k):
            _, id = score_id_pairs[i]
            if id not in document_weights:
                continue
            prf_terms.extend(document_weights[id][2])

        refiner.pseudo_relevance_feedback(prf_terms)
        refined_query = refiner.get_current_refined()
        # End Pseudo-Relevance Feedback

        score_id_pairs = free_text_model.cosine_score(refined_query, k=3500)
