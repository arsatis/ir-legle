...
def tabulate_dictionary(term_dict, doc_weight, entry, zone):
    ...
    weighted_tfs = []
    topk = TopK(5)
    for term, freq in doc_dic.items():
        weighted_tfs.append(1 + math.log10(doc_dic[term]) if doc_dic[term] >= 1 else 0)
        if term not in stop_words and len(term) > 2: # there were lots of full stops and single-letters
            topk.add((freq, term))

    doc_length = math.sqrt(sum([x * x for x in weighted_tfs]))

    topterms = [term for _, term in topk.result()]
    doc_weight[doc_id] = (doc_length, importance, topterms)
